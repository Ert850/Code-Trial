...
Vo = input(float('Enter the initial velocity (m/s): '))
N = input(float('Enter the number of angles: '))

for k in range(N):
    angleD = input(float('Enter the angle (in degrees): '))
    while angleD <= 0:
        angleD = input(float('Enter the angle (in degrees): '))
    while angleD >= 90:
        angleD = input(float('Enter the angle (in degrees): '))
    ...
    maxR = (2(Vo**2)*math.cos(angleA)*math.sin(angleA))/19.62
